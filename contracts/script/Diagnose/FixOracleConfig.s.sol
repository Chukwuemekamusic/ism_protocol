...
/// @notice Script to fix USDC oracle configuration - Increase staleness for testnet (4 days)
/// @dev Run with: forge script script/Diagnose/FixOracleConfig.s.sol:FixUSDCOracle --rpc-url $BASE_SEPOLIA_RPC_URL --broadcast --account testnet
contract FixUSDCOracle is Script {
    ...
    function run() external {
        ...
        console2.log("  Max Staleness:", usdcConfig.maxStaleness, "seconds (4 days)");
        console2.log("");
        ...
        console2.log("\n=== FIX COMPLETE ===");
        console2.log("The USDC oracle now accepts Chainlink data up to 4 days old.");
...
/// @dev Run with: forge script script/Diagnose/FixOracleConfig.s.sol:FixWETHOracle --rpc-url $BASE_SEPOLIA_RPC_URL --broadcast --account testnet
contract FixWETHOracle is Script {
    // Oracle router address from deployment (Base Sepolia)
    address constant ORACLE_ROUTER = 0xD8f530eB3B624c6D4E89d504E48F8b0d00BF0a97;

    // Increased staleness window for testnet (96 hours)
    uint96 constant TESTNET_MAX_STALENESS = 96 hours; // 4 days
    ...
    function run() external {
        ...
        IOracleRouter.OracleConfig memory wethConfig = IOracleRouter.OracleConfig({
            ...
            twapWindow: 0,
            maxStaleness: TESTNET_MAX_STALENESS, // 4 days for testnet
            isToken0: false
        });
        ...
        console2.log("  Max Staleness:", wethConfig.maxStaleness, "seconds (4 days)");
        console2.log("");
        ...
        console2.log("\n=== FIX COMPLETE ===");
        console2.log("The WETH oracle now accepts Chainlink data up to 4 days old.");
        console2.log("This should resolve the staleness issue on Base Sepolia testnet.");
        console2.log("Borrowing should work now!");
    }
}
