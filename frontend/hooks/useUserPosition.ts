...
export function useUserPositions() {
  ...
  const positions: UserPosition[] = markets
    ?.map((marketAddress, index) => {
      const baseIndex = index * 3;
      const supplied = (data?.[baseIndex]?.result as bigint) || 0n;
      const positionData = data?.[baseIndex + 1]?.result as [bigint, bigint] | undefined;
      const borrowed = (data?.[baseIndex + 2]?.result as bigint) || 0n;
