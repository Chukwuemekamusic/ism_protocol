...
/// @dev Adapted from Uniswap V3 Core
library TickMath {
    ...
    /// @return sqrtPriceX96 A Fixed point Q64.96 number representing the sqrt price
    function getSqrtRatioAtTick(int24 tick) internal pure returns (uint160 sqrtPriceX96) {
        ...
        // Divide by 1<<32 and round up to get a Q64.96 number
        sqrtPriceX96 = uint160((ratio >> 32) + (ratio % (1 << 32) == 0 ? 0 : 1));
    }

    /// @notice Calculates the greatest tick value such that getRatioAtTick(tick) <= ratio
    /// @param sqrtPriceX96 The sqrt ratio for which to compute the tick
